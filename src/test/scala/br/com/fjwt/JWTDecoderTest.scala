...
import br.com.fjwt.error.JWTError.{NullPrivateKey, EmptyPrivateKey, NullToken, EmptyToken, ExpiredToken, WrongTokenParts, InvalidSignature, NotMappedError}
...
class JWTDecoderTest extends AnyFlatSpecLike:
  ...
  private given codecValidation: CodecValidation[F] = CodecValidation.dsl
  ...
  private lazy val decoder: JWTDecoder[F] = JWTDecoder.dsl(base64Encoder, hs512Encoder, codecValidation)
