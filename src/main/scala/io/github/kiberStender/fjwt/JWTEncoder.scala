...
trait JWTEncoder[F[*]]:
  /**
   * A method to encode a given payload data described by the type P into a encrypted {@link String} value
   * @param privateKey The key that will be used to encode the payload
   ...
   * @param ZoneId The {@link ZoneID} that will be used to convert the {@link LocalDateTime} parameters to {@link Long} values
   * @tparam P The type of the payload
   * @return The encoded {@link String} or an error wrapped in F
   */
  ...
  /**
   * A method to encode a given payload data described by the type P into a encrypted {@link String value} where you don't have to provide the {@link Claim} manually
   * In this method only issued at(iat) is populated in {@link Claim}
   * @param privateKey The key that will be used to encode the payload
   * @param payload The data you want to transmit itself
   * @param ZoneId The {@link ZoneID} that will be used to convert the {@link LocalDateTime} parameters to {@link Long} values
   * @tparam P The type of the payload
...
/**
 * A factory for {@link JWTEncoder}
 */
object JWTEncoder:
  /**
   * Method to create an instance of {@link JWTEncoder} by providing an instance of {@link Base64Encoder} and {@link HmacEncoder}
   * @param base64Encoder An instance of {@link Base64Encoder} to encode the header and the payload of the token
   * @param hsEncoder An instance of {@link HmacEncoder} to encode both the header and the payload of the token in order to generate the signature of the token
   * @tparam F An instance of {@link MonadError[F, Throwable]}
   * @return An instance of {@link JWTEncoder}
   */
