...
trait StringValidation[F[*]]:
   ...
   * @param nullCase The function to be triggered in case the String is null
   * @param emptyCase The function to be triggered in case the String is empty
   * @tparam E The type to constrain the errors
   * @return Either the valid String or some error describing what went wrong
   */
...
object StringValidation:
  /**
   * A method to create an instance of {@link StringValidation} that requires an instance of {@link ApplicativeError[F, Throwable]}
   * @tparam F An instance of {@link ApplicativeError[F, Throwable]}
   * @return The instance of {@link StringValidation}
   */
